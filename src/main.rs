...
impl BitVec {
    #[inline(always)]
    #[must_use]
    const fn new(bit_mask: u16) -> Self {
    ...
    #[inline(always)]
    #[must_use]
    const fn new_bit(bit_index: u8) -> Self {
    ...
    #[inline(always)]
    fn set(&mut self, mask: BitVec) {
        self.0 |= mask.0;
    }

    #[inline(always)]
    fn clear(&mut self, mask: BitVec) {
        self.0 &= !mask.0;
    }
}
...
impl Board {
    ...
    #[inline(always)]
    #[must_use]
    fn read<U: Iterator<Item = Result<u8, std::io::Error>>>(bytes: &mut U) -> Result<Self, Option<std::io::Error>> {
    ...
    #[inline(always)]
    #[must_use]
    const fn available_values(&self, coords: Coords) -> Option<BitVec> {
    ...
    #[inline(always)]
    fn occupy(&mut self, coords: Coords, value: u8) {
        debug_assert!(coords.r < 9);
        debug_assert!(coords.c < 9);
        debug_assert!(coords.b < 9);
    ...
    #[inline(always)]
    fn leave(&mut self, coords: Coords, value: u8) {
        debug_assert!(coords.r < 9);
        debug_assert!(coords.c < 9);
        debug_assert!(coords.b < 9);
    ...
    #[inline(always)]
    fn solve_and<F: Fn(&Self)>(&mut self, f: F) -> u128 {
